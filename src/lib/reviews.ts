...
export const generateReview = (
  ...
  templateId: overrides?.templateId || `template-${Math.floor(Math.random() * 100)}`,
  userId: overrides?.userId || `user-${Math.floor(Math.random() * 100)}`,
  userName: overrides?.userName || `テストユーザー${Math.floor(Math.random() * 100)}`,
  rating: overrides?.rating || Math.floor(Math.random() * 5) + 1,
  comment: overrides?.comment || `テストレビュー ${index ?? 1 + 1}です。`,
...
export const analyzeReviewSentiment = (content: string): 'positive' | 'negative' | 'neutral' => {
...
export const reviewsApi = {
  ...
  // テンプレートの平均評価を取得
  getTemplateRating: async (templateId: string): Promise<{ average: number; count: number }> => {
  ...
  // ユーザーによるテンプレート評価の登録
  addReview: async (templateId: string, review: ReviewForm): Promise<TemplateReview> => {
