...
jest.mock('@/components/ui/dialog', () => ({
  ...
  DialogContent: ({ children }: { children: React.ReactNode }) => (
    <div data-testid="dialog-content" className="max-w-6xl h-[90vh] flex flex-col p-4 gap-2">
      {children}
    </div>
  ),
...
describe('PreviewModal Component', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should display the template title', () => {
      ...
      expect(screen.getByTestId('dialog-title')).toHaveTextContent('Test Template Preview');
    });

    it('should not render when template is null', () => {
      render(<PreviewModal template={null as Template | null} onClose={mockOnClose} />);

      expect(screen.queryByTestId('dialog')).not.toBeInTheDocument();
    });

    it('should not render when template is undefined', () => {
      const mockTemplate = undefined as unknown as Template;
      render(<PreviewModal template={mockTemplate} onClose={mockOnClose} />);
  ...
  describe('Props', () => {
    it('should receive template props', () => {
      ...
      expect(screen.getByTestId('dialog-title')).toHaveTextContent('Custom Template Preview');
    });
  ...
  describe('Dialog Behavior', () => {
    ...
    it('should set dialog open state based on template presence', () => {
      const { rerender } = render(<PreviewModal template={mockTemplate} onClose={mockOnClose} />);
  ...
  describe('onClose Callback', () => {
    it('should call onClose when component unmounts without null template', () => {
      const { unmount } = render(<PreviewModal template={mockTemplate} onClose={mockOnClose} />);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle template with special characters in title', () => {
      ...
      render(<PreviewModal template={specialCharTemplate} onClose={mockOnClose} />);
    ...
    it('should handle very long template title', () => {
      ...
      render(<PreviewModal template={longTitleTemplate} onClose={mockOnClose} />);

      expect(screen.getByTestId('dialog-title')).toHaveTextContent(`${'A'.repeat(200)} Preview`);
    });
    ...
    it('should handle malformed URL in preview_path', () => {
      ...
      render(<PreviewModal template={malformedUrlTemplate} onClose={mockOnClose} />);
