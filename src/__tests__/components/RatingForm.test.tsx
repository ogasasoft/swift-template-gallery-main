...
describe('RatingForm Component', () => {
  ...
  it('should render all required fields', () => {
    ...
    expect(
      screen.getByPlaceholderText(/このテンプレートについてご意見をお聞かせください.../i)
    ).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /評価を送信/i })).toBeInTheDocument();
  });
  ...
  it('should show submitted message after successful submission', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/このテンプレートの評価を送信しました/i)).toBeInTheDocument();
    });
  });
  ...
  it('should call onReviewSubmitted callback after submission', async () => {
    ...
    render(<RatingForm templateId="test-template" onReviewSubmitted={onReviewSubmitted} />);
    const stars = screen.getAllByRole('button', { name: /stars/i });
    fireEvent.click(stars[3]); // Click 4 stars
    fireEvent.change(
      screen.getByPlaceholderText(/このテンプレートについてご意見をお聞かせください.../i),
